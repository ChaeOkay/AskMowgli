a = User.create(email: 'derp@derp.com', password: 'password', name: 'Derp')
b = User.create(email: 'wolverine@derp.com', password: 'snarl', name: 'The Wolverine')
c = User.create(email: 'dagget@derp.com', password: 'jalapeno', name: 'Dagget')

survey1 = Survey.create(name: "Condiments", user: a)
survey2 = Survey.create(name: "Woodland Creatures", user: b)
survey3 = Survey.create(name: "Rocks", user: c)

question1= Question.create(question_type: 'text', description: 'For use on burgers or sandwiches; for instance, mayo, mustard, relish, ketchup, etc...', prompt: 'Favorite condiment?', survey: survey1)
question2= Question.create(question_type: 'text', description: 'For combat? For instance, perhaps the mighty wolverine...', prompt: 'Which woodland creature is best?', survey: survey2)
question3= Question.create(question_type: 'text', description: 'Start with type, such as igneous, sedimentary, or metamorphic. Need not be of terrestrial origin, either (i.e. moon rocks, meteorites, etc.', prompt: 'What is your favorite rock?', survey: survey3)

reply1 = Reply.create(value: 'mustard', question: question1)
reply2 = Reply.create(value: 'mustard', question: question1)
reply3 = Reply.create(value: 'chili sauce', question: question1)
reply4 = Reply.create(value: 'ketchup', question: question1)
reply5 = Reply.create(value: 'ketchup', question: question1)
reply6 = Reply.create(value: 'ketchup', question: question1)
reply7 = Reply.create(value: 'mayo', question: question1)
reply8 = Reply.create(value: 'elk', question: question2)
reply9 = Reply.create(value: 'vole', question: question2)
reply10 = Reply.create(value: 'opal', question: question3)
reply11 = Reply.create(value: 'skipping stones', question: question3)